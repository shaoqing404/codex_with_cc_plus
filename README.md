...
所有 OpenAI-compatible / DS report worker 都是 advisory only。runner 会在
config/status/stream/report artifacts 中记录 `mayOverrideValidator=false`、
`mayOverrideVerifier=false`、`canEditBusinessFiles=false`、`canRunShellTests=false`、
`canDispatchWorkerRuns=false` 和 `canAcceptWorkflowResults=false`；模型漏写时由
runner 自动补齐。
...
./skills/codex-with-cc/macos_scripts/ccstatus.sh preflight --json
./skills/codex-with-cc/macos_scripts/ccstatus.sh run -RunId <run-id> --json
./skills/codex-with-cc/macos_scripts/ccstatus.sh audit -RunId <run-id> --json
./skills/codex-with-cc/macos_scripts/ccstatus.sh audit -WorkflowId <workflow-id> --json
./skills/codex-with-cc/macos_scripts/ccindex.sh build --json
./skills/codex-with-cc/macos_scripts/ccdash.sh build
```

`ccclean` 是可审计清理工具，不是静默定时任务。默认只计划旧的成功
workflow group；失败、卡死、运行中、最近活动和孤儿 run 会被保护，除非显式
传入 `-IncludeFailures`、`-IncludeRunning` 或 `-IncludeOrphans`。真正移动文件
必须使用 `apply -ConfirmDelete`，并且默认移动到
`$CODEX_HOME/codex_with_cc/cleanup-trash/<timestamp>`，同时写入 cleanup manifest。

`ccruntime` 是受控 runtime 管理入口，用来读取 Claude Code CLI、OpenClaw、
ccwitch、Claude settings 和 runner 默认模型/权限模式。`status` 和
`plan-switch` 是只读/干跑；`apply-switch` 必须显式传入
`-ConfirmRuntimeChange`，只允许修改 Claude settings 白名单字段，并写入
`runtime_<timestamp>.json/.md` 审计 artifacts。API token 只记录存在性，不写入
artifacts。

`ccstatus` 是给 Codex 主线程看的决策入口。`preflight` 在首次实现派工前检查
Claude Code CLI、脱敏 settings、本地 backend 可达性和 artifact root 可写性；
失败时返回 `delegateStatus=REFUSED`、`dispatchAllowed=false` 和下一步修复命令。
`run` / `workflow` 则把 live run、dead process、report ready、missing gates 等状态
翻译成主线程下一步动作，例如等待 60 秒、运行 verifier、重派或取证。
`audit -RunId` 会写入 `audit_<RunId>.json/.md`，把 worker claim、声明测试、观察到
的 verification、scope、缺失 gates、failure layer 和 `acceptanceAllowed` 汇总成主线
程可复查的审计包。`audit -WorkflowId` 会写入 `audit_<WorkflowId>.json/.md`，汇总所有
run audit、failed/running runs、缺失 gates、workflow final acceptance 和下一步动作。
`verify_delegate_workflow.*` 还会自动写入 `verifier_audit_<WorkflowId>.json/.md`，
记录 deterministic gate results、verified runs、failed gate 和 `acceptanceAllowed`；
这是 verifier 自己的验收证据，不能覆盖 `ccstatus` rollup 或最终人工判断。

`ccindex` 和 `ccdash` 是机器级观察层。`ccindex` 扫描项目 artifact root 和
user fallback roots，输出 workflow/run、状态、模型、权限、置信度和 provenance。
`ccdash` 基于同一份 JSON 生成本地静态只读 HTML 看板。本阶段不启动 Docker 或后台
API 服务。

子线程不能只把 artifact paths 打印给主线程就结束。标准返回必须告诉主线程当前
状态和下一步动作：
...
只有终态 artifact、worker report 和 deterministic verifier 都成立时，子线程才
能返回 `DelegateStatus: TERMINAL` 并建议主线程进入 review 或 workflow verification。
...
| Claude Code CLI | `2.1.165` | `>= 2.1.141` 或最新版 | 真实 `delegate_to_claude` worker 执行 |
...
./skills/codex-with-cc/macos_scripts/ccindex.sh list
./skills/codex-with-cc/macos_scripts/ccindex.sh show <workflow-id>
./skills/codex-with-cc/macos_scripts/ccdash.sh build
```

> [!WARNING]
> **⚠️ 仓库迁移与归档通告**
> **本仓库 `shaoqing404/codex_with_cc` 已归档并停止维护。**
> 为了摆脱原 Fork 属性限制并独立迭代开发，所有的功能迭代与问题反馈已完全移至全新的独立非 Fork 仓库：
> 👉 **[shaoqing404/codex_with_cc_plus](https://github.com/shaoqing404/codex_with_cc_plus)**
> 所有的集成、手动安装、环境配置与新代码开发均在新仓库中完成。
...
> [!CAUTION]
> **关于 Marketplace 直接安装的失败提示**：
> 如果你尝试直接使用 `codex plugin marketplace add shaoqing404/codex_with_cc` 会遇到报错：`marketplace root does not contain a supported manifest`。
> 这是因为本仓库是作为独立的高级委派 Skill 设计的，并不属于 Codex Marketplace 多插件源的根树。
> **官方唯一的正确集成与安装方式是：拉取本仓库并进行本地手动安装。**
